...
def test_price_valid_query(client):
    ...
    assert client.get(f"/prices/spot?currency_pairs={pairs}").status_code == 200

def test_price_invalid(client):
    pairs = ".btc_usd,None_Btc,do_t_eth,test,tes_ting!,"
    assert client.get(f"/prices/spot?currency_pairs={pairs}").status_code == 404

def test_price_response_type(client):
    pairs = "ksm_dot,kyl_usdt,bnb_gbp"
    response = json.loads(client.get(f"/prices/spot?currency_pairs={pairs}").data.decode("utf8"))
